...
- 打开「设置 → 历史」：「消息记录」浏览与搜索本地聊天；「统计概览」查看消息趋势、内容分布与活跃时段。
- 部分旧版入口：右键模型 →「历史」仍可打开历史相关能力（以当前版本菜单为准）。

### 4.5 数据与配置维护

- 「设置 → 高级 → 数据管理」：查看本地数据库、内嵌媒体、模型库与日志占用；清理缓存、导出日志、导入/导出应用配置、下载 Live2D Core 等。
- 可用 `Ctrl/Cmd+K` 在设置内快速跳转到各子页。
